  ...
  class ProductNewAttribute < Mage::Base
    ...
    def find_attribute(attribute_code)
      ...
      if attribute.nil?
        ...
        :attribute_model => nil,  
        :backend_model => '',    
        :backend_type => checked?(att[:is_selectable]) ? 'int' : 'varchar',
        :backend_table => '',
        :frontend_model => '', 
        :frontend_input => checked?(att[:is_selectable]) ? 'select' : 'text' ,
        :frontend_label => attribute_code.humanize,
        :frontend_class => '', 
        :source_model => checked?(att[:is_selectable]) ? 'eav/entity_attribute_source_table' : '' ,
        :is_required => checked?(att[:is_required]), 
        :is_user_defined => 1, 
        :default_value => '', 
        :is_unique => 0, 
        :is_required => 0,
